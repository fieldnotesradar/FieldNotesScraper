...
class MySpider(SitemapSpider):
    ...
    def parse(self, response):
        yield {
            ...
            'url': response.url,
            'name': response.xpath('//*[@class="panel__title"]/text()').get().strip(),
            'details': response.xpath('//*[@class="panel__details"]/span/text()').get(),
            'packaging': response.xpath('//*[@class="panel__sub-title"][1]/text()').get().strip(),
            'dimensions': response.xpath('//*[@class="panel__sub-title"][2]/text()').get().strip(),
